...
build {
  ...
  # Download the MicroOS x86 image and verify its SHA256 checksum (F25)
  provisioner "shell" {
    inline = [
      "${local.download_image}${var.opensuse_microos_x86_mirror_link}",
      "${local.download_image}${var.opensuse_microos_x86_mirror_link}.sha256",
      "sha256sum -c *.sha256 || { echo 'MicroOS x86 image checksum verification failed'; exit 1; }",
    ]
  }
...
build {
  ...
  # Download the MicroOS ARM image and verify its SHA256 checksum (F25)
  provisioner "shell" {
    inline = [
      "${local.download_image}${var.opensuse_microos_arm_mirror_link}",
      "${local.download_image}${var.opensuse_microos_arm_mirror_link}.sha256",
      "sha256sum -c *.sha256 || { echo 'MicroOS ARM image checksum verification failed'; exit 1; }",
    ]
  }
